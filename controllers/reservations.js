    ...
    try {
        ...
        if (req.user.role === 'user') {
            ...
            if (req.params.coworkingSpaceId) {
                ...
                reservations = await Reservation.findAll({ include: CoworkingSpace.name });
            }
        }
...
exports.addReservation = async (req, res, next) => {
    try {
        ...
        const coworkingspace = await CoworkingSpace.findByPk(req.params.coworkingSpaceId);
        if (!coworkingspace) {
        ...
        res.status(500).json({ success: false, message: 'Cannot create a reservation' });
    }
};
...
exports.updateReservation = async (req, res, next) => {
    try {
        ...
        if (reservation.dataValues.userId !== req.user.id && req.user.role !== 'admin') {
            return res.status(401).json({ success: false, message: `user ${req.user.id} is not authorized to update this reservation` });
        }

        await reservation.update(req.body);
        const modifiedReservation = await Reservation.findByPk(req.params.id);
        res.status(200).json({ success: true, data: modifiedReservation.dataValues });
    } catch (err) {
        res.status(500).json({ success: false, message: 'Cannot update a reservation' });
    }
};
...
exports.deleteReservation = async (req, res, next) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Cannot delete a reservation' });
    }
};
