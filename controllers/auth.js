...
exports.register = async (req, res, next) => {
    try {
        ...
        res.status(201).json({ success: true, email: user.email, activateAccountToken: user.activateAccountToken });
    } catch (err) {
...
exports.login = async (req, res, next) => {
    try {
        ...
        if (!user.isActive) {
            return res.status(400).json({ success: false, message: 'Please activate the account' });
        }
